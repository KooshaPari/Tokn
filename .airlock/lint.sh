...
if [[ -n "$RUST_FILES" ]]; then
  ...
  echo "$RUST_FILES" | xargs rustfmt --edition 2021 || true
  echo "rustfmt done."

  echo ""
  echo "=== Rust linting (clippy) ==="

  # Lint check — fail on warnings
  # Using AllowByDefault for certain clips as per project .clippy.toml
  echo "$RUST_FILES" | xargs cargo clippy \
