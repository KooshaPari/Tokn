...
if [[ -n "$RUST_FILES" ]]; then
  echo "==> Rust: cargo fmt (format)"
  cargo fmt --all || true
  echo ""

  echo "==> Rust: cargo clippy (lint with auto-fix)"
  if ! cargo clippy --all-targets --all-features --fix --allow-dirty --allow-staged 2>&1; then
    EXIT_CODE=1
  fi
  echo ""

  echo "==> Rust: cargo clippy (check)"
